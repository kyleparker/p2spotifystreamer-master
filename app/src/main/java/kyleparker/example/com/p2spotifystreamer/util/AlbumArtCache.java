...
public final class AlbumArtCache {
    ...
    public void fetch(final String url, final FetchListener listener) {
        ...
        if (bitmap != null) {
            LogHelper.d(TAG, "getOrFetch: album art is in cache, using it", url);
            listener.onFetched(url, bitmap[BIG_BITMAP_INDEX], bitmap[ICON_BITMAP_INDEX]);
            return;
        }
        LogHelper.d(TAG, "getOrFetch: starting asynctask to fetch ", url);
        ...
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                Bitmap[] bitmaps;
                try {
                    Bitmap bitmap = BitmapHelper.fetchAndRescaleBitmap(url, MAX_ART_WIDTH, MAX_ART_HEIGHT);
            ...
            protected void onPostExecute(Bitmap[] bitmaps) {
                if (bitmaps == null) {
                    ...
                    listener.onFetched(url, bitmaps[BIG_BITMAP_INDEX], bitmaps[ICON_BITMAP_INDEX]);
                }
            }
        }.execute();
    }
